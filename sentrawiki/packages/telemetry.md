...
- **`PrismaInstrumentation`** is started by `@safrs/telemetry`'s `initTelemetry`, invoked from `projects/internal/golden-path/apps/web/src/instrumentation.ts` — not from `packages/database/src/client.ts`, which only builds the Prisma client.
