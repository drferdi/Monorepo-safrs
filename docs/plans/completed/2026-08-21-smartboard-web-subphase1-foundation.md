...
- Task 4: 1 fix round — varian `size="sm"` tombol awalnya `min-h-(--space-6)` (32px), melanggar `packages/token/UI-RULES.md` (44px minimum target, build condition). Diperbaiki ke `var(--target-min)`.
...
2. `pnpm task list --active` bersih dari overlap scope `projects/academic/academic-smartboard/`. Verifikasi ulang di Task 1 — per audit 2026-08-21 tidak ada lease di scope ini (lease aktif lain: `TASK-20260821-SENTRABOT-*` dan seri `TASK-20260820-KILO-*`/`REMOVE-CLINE*`, semua di luar `projects/academic/academic-smartboard/`).
...
- File kontrol verifikasi (`projects/academic/academic-smartboard/AGENTS.md`, `packages/token/scope.txt`, `.github/workflows/**`) TIDAK boleh satu change set dengan implementasi — branch kontrol terpisah (Task 11), SETELAH implementasi merged + pushed.
...
```text
projects/academic/academic-smartboard/apps/web/
...
**Files:** Commit: `docs/plans/active/2026-08-21-smartboard-web-roadmap.md`, `docs/plans/active/2026-08-21-smartboard-web-subphase1-foundation.md`, baris di `docs/plans/active/README.md`
...
Pastikan tidak ada lease baru yang menabrak `projects/academic/academic-smartboard/` sejak audit 2026-08-21 di atas.
...
**Files:** Create: `projects/academic/academic-smartboard/apps/web/{package.json,next.config.ts,tsconfig.json,postcss.config.mjs}`; Create: `projects/academic/academic-smartboard/apps/web/src/app/{layout.tsx,page.tsx,globals.css}`; Modify: `turbo.json` (tambah `NEXT_PUBLIC_BACKEND_URL`, `NEXT_PUBLIC_DEV_TENANT_SLUG` ke array `env` task `build` dan `dev`)

**Interfaces:** Produces: package `@sentra/smartboard-web`; `pnpm --filter @sentra/smartboard-web build` menghasilkan `apps/web/out/`
...
- [x] **Step 3: tsconfig.json** — identik `projects/academic/academic-smartboard/apps/site/tsconfig.json` (extends `packages/config/tsconfig/nextjs.json`).
...
ls projects/academic/academic-smartboard/apps/web/out/index.html
```
...
```bash
git add projects/academic/academic-smartboard/apps/web turbo.json pnpm-lock.yaml
git commit -m "feat(web): scaffold apps/web next static export"
```

### Task 4: Primitif UI token-based (TDD util `cn`)

**Files:** Create: `src/lib/{cn.ts,cn.test.ts}`, `src/components/ui/{button.tsx,label.tsx,input.tsx}`

**Interfaces:** Produces: `cn(...classes)`, `<Button variant="default"|"outline"|"ghost" size="default"|"sm">`, `<Label>`, `<Input>` — dipakai semua task berikutnya

- [x] **Step 1: Test gagal** — `projects/academic/academic-smartboard/apps/web/src/lib/cn.test.ts`:
...
- [x] **Step 4**: Test PASS.
- [x] **Step 5**: `projects/academic/academic-smartboard/apps/web/src/components/ui/button.tsx` (varian via `class-variance-authority`, warna via token semantik `var(--color-*)` — bukan Tailwind palette default), `projects/academic/academic-smartboard/apps/web/src/components/ui/label.tsx` (bungkus `@radix-ui/react-label`), `projects/academic/academic-smartboard/apps/web/src/components/ui/input.tsx` (native `<input>` + kelas token border/radius). Semua tanpa hex mentah.
- [x] **Step 6**: `pnpm --filter @sentra/smartboard-web lint && pnpm --filter @sentra/smartboard-web typecheck && pnpm --filter @sentra/smartboard-web build` → hijau. Grep bukti nol hex: `grep -rnE "#[0-9a-fA-F]{3,8}\b" projects/academic/academic-smartboard/apps/web/src` → kosong.
- [x] **Step 7**: Commit `feat(web): token-based ui primitives (button, label, input)`.

### Task 5: API client (TDD)

**Files:** Create: `src/lib/{api.ts,api.test.ts}`

**Interfaces:** Produces: `apiClient` (axios instance), `type User = { user_id, tenant_id, email, name, role, tutor_id, student_ids, parent_id, active }`, `login(email, password): Promise<{user: User}>`, `getMe(): Promise<User>`, `logout(): Promise<void>`

- [x] **Step 1: Test gagal** — `projects/academic/academic-smartboard/apps/web/src/lib/api.test.ts` (unit murni, mock axios, tanpa network/DOM):
...
- [x] **Step 3: Implementasi** — `projects/academic/academic-smartboard/apps/web/src/lib/api.ts`:
...
**Files:** Create: `src/lib/{auth.tsx,auth.test.ts}`, `src/app/login/page.tsx`

**Interfaces:** Consumes: `login`, `getMe`, `logout`, `User` dari Task 5. Produces: `<AuthProvider>`, `useAuth(): { user: User | null, status: "loading"|"authenticated"|"unauthenticated", login, logout }` — dipakai `ProtectedRoute` (Task 7) dan semua halaman terproteksi

- [x] **Step 1: Test gagal** — `projects/academic/academic-smartboard/apps/web/src/lib/auth.test.ts` (test reducer/state machine murni, tanpa render React):
...
- [x] **Step 3: Implementasi** — `projects/academic/academic-smartboard/apps/web/src/lib/auth.tsx`:
...
**Files:** Create: `src/lib/{nav.ts,nav.test.ts}`, `src/components/{ProtectedRoute.tsx,AppShell.tsx}`; Modify: `src/app/layout.tsx` (pasang `AuthProvider` + `QueryClientProvider`), `src/app/page.tsx` (redirect client berbasis auth)

**Interfaces:** Consumes: `useAuth()` dari Task 6. Produces: `NAV_ITEMS: {label, href, roles}[]`, `filterByRole(items, role)`, `<ProtectedRoute roles={[...]}>`, `<AppShell>`

- [x] **Step 1: Test gagal** — `projects/academic/academic-smartboard/apps/web/src/lib/nav.test.ts`:
...
- [x] **Step 2**: FAIL → implementasi `projects/academic/academic-smartboard/apps/web/src/lib/nav.ts`:
...
**Files:** Create: `src/components/DataTable.tsx`, `src/app/master/murid/page.tsx`

**Interfaces:** Consumes: `listStudents`, `Student` (Task 5), `ProtectedRoute`, `AppShell` (Task 7), `@tanstack/react-query`
...
- [x] **Step 2**: `projects/academic/academic-smartboard/apps/web/src/components/DataTable.tsx` — tabel generik minimal (`columns: {key, header}[]`, `rows: Record<string, unknown>[]`), token-styled (`var(--color-border)`, dst.), TANPA fitur sort/filter/paginasi (YAGNI — nambah kalau halaman sub-fase 2-5 butuh).
- [x] **Step 3**: `projects/academic/academic-smartboard/apps/web/src/app/master/murid/page.tsx` — client component: `<ProtectedRoute roles={["owner","admin_akademik","tentor","murid_ortu"]}>`, `useQuery({ queryKey: ["students"], queryFn: listStudents })`, render `<AppShell>` + `<DataTable columns=... rows=data ?? []>` kolom: Nama, NIS, Jenis Kelamin, Status. Loading state dan error state (pesan "Gagal memuat data murid" + tombol retry `refetch()`).
...
**Files:** Create: `tests/build-output.test.mjs`
...
- [x] **Step 3**: Audit token: `node scripts/check-tokens.mjs --audit` → nol pelanggaran pada `projects/academic/academic-smartboard/apps/web`.
...
**Files:** Modify: `projects/academic/academic-smartboard/{README.md,docs/architecture.md,docs/data.md,docs/testing.md}` (status `apps/web`: `di-port (sub-fase 1/5)`; JANGAN sentuh `AGENTS.md` — itu Task 11); Modify: `docs/plans/active/2026-08-21-smartboard-web-subphase1-foundation.md` (centang task; catatan eksekusi), `docs/plans/active/2026-08-21-smartboard-web-roadmap.md` (baris 1 status → COMPLETED sub-fase)
...
**Files (SEMUA kontrol verifikasi):** Modify: `projects/academic/academic-smartboard/AGENTS.md` (tambah perintah `apps/web` di section Commands, di samping `apps/site` yang sudah ada); Create: `projects/academic/academic-smartboard/apps/web/AGENTS.md`; Modify: `packages/token/scope.txt` (+ `projects/academic/academic-smartboard/apps/web/src`)
...
- [x] **Step 4**: Tambah baris `projects/academic/academic-smartboard/apps/web/src` ke `packages/token/scope.txt`.
