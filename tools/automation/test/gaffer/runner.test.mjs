...
function basePorts(overrides = {}) {
  return {
    ...
    executeWorker: async () => ({ status: 'complete', changedFiles: ['src/t1.ts'] }),
    verify: async () => ({ passed: true }),
    rootAccept: async () => ({ accepted: true }),
    listAvailableWorkers: async ({ requestedClass }) => [{ id: `worker-${requestedClass}`, class: requestedClass, classes: [requestedClass], certified: true, expectedCost: 1 }],
    ...overrides,
  };
}

test('SOLO path reaches READY', async () => {
  const result = await runGaffer({ intent: 'small change', ports: basePorts() });
...
test('decomposed task executes economy and verifies', async () => {
  ...
  const ports = basePorts({
    rootPlan: async () => ({
      ...
      tasks: [{
        id: 'T1', objective: 'implement', acceptance: [], ownedPaths: ['src/t1.ts'], verification: ['test'], dependencies: [], architectureSettled: true,
      }],
    }),
...
test('capability mismatch escalates economy to strong', async () => {
  ...
  const ports = basePorts({
    rootPlan: async () => ({
      route: Route.DECOMPOSE,
      tasks: [{ id: 'T1', objective: 'implement', acceptance: [], ownedPaths: ['src/t1.ts'], verification: ['test'], dependencies: [], architectureSettled: true }],
    }),
    executeWorker: async ({ workerClass }) => {
      calls.push(workerClass);
      return { status: 'complete', changedFiles: ['src/t1.ts'] };
    ...
    verify: async ({ mode }) => {
      ...
      verificationCount += 1;
      if (verificationCount === 1) return { passed: false, failureClass: FailureClass.CAPABILITY_MISMATCH };
      return { passed: true };
    },
  });
  const result = await runGaffer({ intent: 'feature', ports });
...
test('scope violation prevents readiness', async () => {
  const ports = basePorts({
    ...
    executeWorker: async () => ({ status: 'complete', changedFiles: ['package.json'] }),
  });
  const result = await runGaffer({ intent: 'feature', ports, policy: { maxAttemptsPerTask: 1 } });
