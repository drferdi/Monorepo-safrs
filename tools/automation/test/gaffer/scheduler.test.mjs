...
import { createTaskGraph, refreshTaskReadiness, readyTasks } from '../../src/gaffer/scheduler.mjs';
...
test('root dependency becomes ready first', () => {
  ...
  assert.deepEqual(readyTasks(graph).map((x) => x.id), ['T1']);
});
...
test('cycles are rejected', () => {
  assert.throws(() => createTaskGraph([task('T1', ['T2']), task('T2', ['T1'])]), /cycle/i);
});
