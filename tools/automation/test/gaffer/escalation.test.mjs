...
test('capability mismatch escalates economy immediately', () => {
  assert.deepEqual(
    escalationDecision({ failureClass: FailureClass.CAPABILITY_MISMATCH, currentClass: WorkerClass.ECONOMY, attempts: 1 }),
...
test('spec error allows one economy correction', () => {
  assert.deepEqual(
    escalationDecision({ failureClass: FailureClass.SPEC_ERROR, currentClass: WorkerClass.ECONOMY, attempts: 1, maxAttempts: 2 }),
    { action: 'RETRY_SAME', workerClass: WorkerClass.ECONOMY },
  );
});

test('scope violation blocks', () => {
  const result = escalationDecision({ failureClass: FailureClass.SCOPE_VIOLATION, currentClass: WorkerClass.ECONOMY, attempts: 1 });
