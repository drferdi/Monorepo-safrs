...
export function resolveWorker({
  ...
  const candidates = availableWorkers
    ...
    .sort((a, b) => {
      const aCost = Number.isFinite(a.expectedCost)
        ? a.expectedCost
        : (costRank[requestedClass] ?? 9999);
      const bCost = Number.isFinite(b.expectedCost)
        ? b.expectedCost
        : (costRank[requestedClass] ?? 9999);
...
export function assertNoSilentSubstitution(requestedClass, actualClass) {
  if (requestedClass !== actualClass) {
    const error = new Error(
      `Worker class substitution requires explicit reroute: requested=${requestedClass} actual=${actualClass}`,
    );
