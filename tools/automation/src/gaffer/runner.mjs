import { classifyWorker, selectExecutionRoute } from "./classifier.mjs";
...
import { assertNoSilentSubstitution, resolveWorker } from "./router.mjs";
import {
  allVerified,
  createTaskGraph,
  readyTasks,
...
function requiredFunction(ports, name) {
  if (typeof ports?.[name] !== "function")
    throw new TypeError(`Gaffer port '${name}' must be a function`);
  return ports[name];
}

function errorToFailureClass(error) {
  if (error?.code && Object.values(FailureClass).includes(error.code))
    return error.code;
  if (error?.code === "ADAPTER_UNAVAILABLE")
    return FailureClass.ADAPTER_UNAVAILABLE;
 ...
 */
export async function runGaffer({
  intent,
  context = {},
  policy = {},
  ports,
  telemetry: suppliedTelemetry = null,
}) {
  ...
  const maxAttempts = Number.isInteger(policy.maxAttemptsPerTask)
    ? policy.maxAttemptsPerTask
    : 2;
  const maxWorkerCalls = Number.isInteger(policy.maxWorkerCalls)
    ? policy.maxWorkerCalls
    : 12;
  let workerCalls = 0;
  ...
  if (authorization?.allowed === false) {
    await telemetry.emit("gaffer.run.blocked", {
      reason: authorization.reason ?? "SAFRS authorization denied",
    });
    return {
      status: "BLOCKED",
      reason: authorization.reason ?? "SAFRS authorization denied",
  ...
  await telemetry.emit("gaffer.route.selected", {
    route,
    project: project?.id ?? project?.path ?? null,
  });
  ...
  if (route === Route.SOLO) {
    ...
    const acceptance = await rootAccept({
      intent,
      project,
      plan,
      verification,
    ...
    if (!acceptance?.accepted) {
      await telemetry.emit("gaffer.run.blocked", {
        reason: acceptance?.reason ?? "root_rejected",
      });
      return {
        status: "BLOCKED",
        reason: acceptance?.reason ?? "root_rejected",
  ...
  while (!allVerified(tasks)) {
    ...
    if (ready.length === 0) {
      const unresolved = [...tasks.values()].filter(
        (t) => t.state !== TaskState.VERIFIED,
      );
      return {
        status: "BLOCKED",
    ...
    const safrsTaskDecision = await authorize({
      intent,
      project,
      task,
      context,
    ...
    if (safrsTaskDecision?.allowed === false) {
      updateTask(tasks, task.id, {
        ...
        failureClass: FailureClass.SECURITY_RISK,
      });
    ...
    let requestedClass =
      task.workerClass ?? classifyWorker(task, safrsTaskDecision);
    ...
    while (!taskComplete) {
      if (workerCalls >= maxWorkerCalls) {
        updateTask(tasks, task.id, {
          state: TaskState.BLOCKED,
          failureClass: FailureClass.BUDGET_EXHAUSTED,
        });
        await telemetry.emit("gaffer.run.blocked", {
          reason: FailureClass.BUDGET_EXHAUSTED,
          taskId: task.id,
        });
        return {
          status: "BLOCKED",
      ...
      const availableWorkers = await listAvailableWorkers({
        requestedClass,
        task,
        project,
      });
      ...
      try {
        ...
        assertNoSilentSubstitution(
          requestedClass,
          worker.class ?? requestedClass,
        );
      } catch (error) {
        updateTask(tasks, task.id, {
          state: TaskState.BLOCKED,
          failureClass: FailureClass.ADAPTER_UNAVAILABLE,
        });
        await telemetry.emit("gaffer.run.blocked", {
          reason: FailureClass.ADAPTER_UNAVAILABLE,
          taskId: task.id,
        });
        return {
          status: "BLOCKED",
      ...
      workerCalls += 1;
      await telemetry.emit("gaffer.worker.selected", {
        taskId: task.id,
        workerClass: requestedClass,
        workerId: worker.id,
      });
      ...
      try {
        ...
        const verification = await verify({
          ...
          task,
          workerResult,
          authorization: safrsTaskDecision,
        });

        if (!verification?.passed) {
          const failureClass =
            verification?.failureClass ?? FailureClass.TEST_FAILURE;
          ...
          await telemetry.emit("gaffer.verification.completed", {
            taskId: task.id,
            passed: false,
            failureClass,
          });
          ...
          if (decision.action === "ESCALATE") {
            ...
            await telemetry.emit("gaffer.escalation.triggered", {
              taskId: task.id,
              to: requestedClass,
              failureClass,
            });
        ...
        await telemetry.emit("gaffer.task.verified", {
          taskId: task.id,
          workerClass: requestedClass,
        });
        taskComplete = true;
  ...
  if (authorization?.requiresAudit) {
    ...
    await telemetry.emit("gaffer.audit.completed", {
      verdict: auditResult?.verdict ?? null,
    });
    if (auditResult?.verdict !== "SHIP") {
      return {
        status: "BLOCKED",
        reason: `audit_${String(auditResult?.verdict ?? "UNKNOWN").toLowerCase()}`,
        audit: auditResult,
        telemetry: telemetry.events,
      };
    }
