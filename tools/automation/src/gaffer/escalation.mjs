...
export function escalationDecision({
  ...
  if (
    ...
    if (currentClass === WorkerClass.ECONOMY)
      return { action: "ESCALATE", workerClass: WorkerClass.STRONG };
    return { action: "BLOCK", reason: failureClass };
  }
  if (
    failureClass === FailureClass.SPEC_ERROR &&
    currentClass === WorkerClass.ECONOMY &&
    attempts < Math.min(maxAttempts, 2)
  ) {
  ...
  if (
    [
      ...
      FailureClass.BLOCKED_REQUIREMENT,
